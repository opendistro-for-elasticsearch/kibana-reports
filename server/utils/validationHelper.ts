...
// Original Regex for URL Validation
// export const regexRelativeUrl = /^\/(_plugin\/kibana\/app|app)\/(dashboards|visualize|discover)#\/(view|edit)\/[^\/]+$/;
//const regexRelativeUrl = /^\/(\S+\/app)\/(dashboards|visualize|discover)#\/(view|edit)\/[^\/]+$/;
//More generic PATH
const regexRelativeUrl = /^(\/\S+)?(\/app)\/(dashboards|visualize|discover)#\/(view|edit)\/[^\/]+$/;
...
export const validateReport = async (
  ...
  await validateSavedObject(client, queryUrl, reportSource, kibana_index);
  return report;
};
...
export const validateReportDefinition = async (
  ...
  // Check if saved object actually exists
  await validateSavedObject(client, baseUrl, reportSource, kibana_index);
