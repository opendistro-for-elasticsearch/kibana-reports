...
export const createReport = async (
  ...
  try {
    ...
    if (reportSource === REPORT_TYPE.savedSearch) {
      ...
      // compose url
      let completeQueryUrl:any;
      if(report.query_url.startsWith(basePath)){
        completeQueryUrl = `${baseUri}${report.query_url}`;
      } else {
        completeQueryUrl = `${baseUri}${basePath}${report.query_url}`;
      }
