...
export const createVisualReport = async (
  ...
  page.setDefaultTimeout(600000); // use 60s timeout instead of default 30s
  if (cookie) {
  ...
  // GEt all cookies and check
  /*
  const p = await page.goto(queryUrl, { waitUntil: 'networkidle0' });
  console.log(p);
  const cookies = await page._client.send('Network.getAllCookies');
  cookies.cookies.map((item: any) => {
    if (item.name === SECURITY_AUTH_COOKIE_NAME) {
      console.log(item);
    }
  });
  */
  
  //TODO chromium hangs here if kibana is behind a proxy with basePath
