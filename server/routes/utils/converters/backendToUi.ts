...
export const backendToUiReport = (
  ...
  const baseUrl = getBaseUrl(sourceType, sourceId, basePath);
...
export const backendToUiReportsList = (
  ...
  const res = backendReportsList.map((backendReport) => {
    return { _id: backendReport.id, _source: backendToUiReport(backendReport, basePath) };
  });
  return res;
};
...
const getBaseUrl = (sourceType: BACKEND_REPORT_SOURCE, sourceId: string, basePath: string) => {
  //TODO: AES domain has different prefix, need figure out a general solution
  let baseUrl = `${URL_PREFIX_DICT[sourceType]}${sourceId}`;
  if(!baseUrl.startsWith(basePath)) {
