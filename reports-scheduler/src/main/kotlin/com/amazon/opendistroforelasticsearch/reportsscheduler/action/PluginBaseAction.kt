...
abstract class PluginBaseAction<Request : ActionRequest, Response : ActionResponse>(
    ...
    override fun doExecute(
        ...
        scope.launch {
            try {
                Metrics.getInstance().getNumericalMetric(MetricName.REQUEST_TOTAL).increment()
                Metrics.getInstance().getNumericalMetric(MetricName.REQUEST_INTERVAL_COUNT).increment()
                listener.onResponse(executeRequest(request, user))
                // TODO: Do SUCCESS metric goes here or in RestResponseToXContentListener?
            } catch (exception: ElasticsearchStatusException) {
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_ES_STATUS_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_ES_SECURITY_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_VERSION_CONFLICT_ENGINE_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_INDEX_NOT_FOUND_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_INVALID_INDEX_NAME_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_ILLEGAL_ARGUMENT_EXCEPTION).increment()
                log.warn("$LOG_PREFIX:IllegalArgumentException:", exception)
                listener.onFailure(ElasticsearchStatusException(exception.message, RestStatus.BAD_REQUEST))
            } catch (exception: IllegalStateException) {
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_ILLEGAL_STATE_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_IO_EXCEPTION).increment()
                ...
                Metrics.getInstance().getNumericalMetric(MetricName.REPORT_EXCEPTIONS_INTERNAL_SERVER_ERROR).increment()
